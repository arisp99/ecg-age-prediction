...
if __name__ == "__main__":
    ...
    # Train / val split
    valid_mask = (
        np.arange(len(df)) <= args.n_valid
    )  # takes the first data points for validation. It would be ideal for this to change every iteration I would think...
    train_mask = ~valid_mask
    # weights
    weights = compute_weights(ages)
    # Dataloader
    train_loader = BatchDataloader(
        traces, ages, weights, bs=args.batch_size, mask=train_mask
    )
    valid_loader = BatchDataloader(
        traces, ages, weights, bs=args.batch_size, mask=valid_mask
    )
    tqdm.write("Done!")
    ...
    for ep in range(start_epoch, args.epochs):
        ...
        # Print message
        tqdm.write(
            "Epoch {:2d}: \tTrain Loss {:.6f} "
            "\tValid Loss {:.6f} \tLearning Rate {:.7f}\t".format(
                ep, train_loss, valid_loss, learning_rate
            )
        )
        # Save history
        history = pd.concat(

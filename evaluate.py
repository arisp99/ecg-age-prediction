...
if __name__ == "__main__":
    ...
    parser.add_argument("mdl", type=str, help="folder containing model.")
    parser.add_argument(
        "path_to_traces",
        type=str,
        default="../data/ecg_tracings.hdf5",
        help="path to hdf5 containing ECG traces",
    )
    parser.add_argument(
        "--batch_size", type=int, default=8, help="number of exams per batch."
    )
    parser.add_argument(
        "--output", type=str, default="predicted_age.csv", help="output file."
    )
    parser.add_argument(
        "--traces_dset", default="tracings", help="traces dataset in the hdf5 file."
    )
    parser.add_argument("--ids_dset", help="ids dataset in the hdf5 file.")
    ...
    # Get checkpoint
    ckpt = torch.load(
        os.path.join(args.mdl, "model.pth"), map_location=lambda storage, loc: storage
    )
    # Get config
    ...
    model = ResNet1d(
        ...
        blocks_dim=list(
            zip(config_dict["net_filter_size"], config_dict["net_seq_lengh"])
        ),
        n_classes=1,
